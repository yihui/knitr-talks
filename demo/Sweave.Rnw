...
This is a \textbf{demo} for using the \verb@Sweave@ command in R.  To
get make a regular \LaTeX\ file (like this one) but
...
\verb+@+
\end{tabbing}
where the \verb@include=FALSE@ indicaes that the output (text and graphics)
...
So making a figure is somewhat more complicated in some ways but much simpler
